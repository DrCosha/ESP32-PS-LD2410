...
// ----------- режим компиляции  -----------
// #define DEBUG_IN_SERIAL                          // компиляция в режиме с выводом в порт отладочных сообщений


// Если секция c локальными секретами вынесена в файл "local.h" в котором содержатся "правильные" значения
// то нужно определить условную переменную компиляции LOCAL_SECRET, иначе будут использованы определения из
// основного файла с кодом программы. Это нужно для возможности исключения доступа к чувствительной информации на Git-е.
#define LOCAL_SECRET

#ifdef LOCAL_SECRET
  #include "local.h"
#else
  // ниже определены значения, которые можно переопределить в файле local.h и не включать в общий доступ на Git-е
  ...
  #define LWT_TOPIC   "diy/my_device_name/LWT"      // топик публикации доступности устройства
  #define SET_TOPIC   "diy/my_device_name/set"      // топик публикации команд для устройства
  #define STATE_TOPIC "diy/my_device_name/state"    // топик публикации состояния устройства
  //
#endif
...
#define PIN_SENSOR_TX 17                          // Tx сенсора LD2410
#define PIN_SENSOR_RX 5                           // Rx сенсора LD2410

// константа определяющая промежуток времени между событиями публикации статуса в MQTT
#define MQTT_COLD_LAG 60000                       // сколько миллисекунд должно пройти между выводом состояния устройства в MQTT при отсутсвии сигнала сенсора (report)
#define MQTT_HOT_LAG 3000                         // сколько миллисекунд должно пройти между событиями (report) если датчик определяет присутсвие, но остальные параметры не меняются
  ...
  if (radar_sensor.isConnected() and ((millis()-LastSensorRead) > SENSOR_READ_DELAY))
    { // если сенсор на связи и задержка чтения истекла, то переносим значения во внутренние переменные
      ...
      if (radar_sensor.movingTargetDetected() != HasMoving) {                           
        ...
        if (HasMoving) { HasChanges = true; }                         // если есть движение - нужно сразу об этом сообщить - иначе сообщим по таймауту
      }
...
void report_to_topicMQTT() { // --- пишем события и состояние в MQTT топик ---
  ...
  if (HasChanges or (MQTT_STATUS_LAG < (millis()-LastReportToMQTT))) { // если есть изменения или наступил таймаут по публикации статуса в MQTT и при этом мы не в циклических изменениях
    if (mqttClient.connected()) {                               // если есть соединение с MQTT - выкладываем статус устройства в MQTT
        ...
        LastReportToMQTT = millis();              // запоминаем текущий момент времени
...
void setup() {  // --- процедура начальной инициализации устройства ---
  ...
  Serial2.begin (256000, SERIAL_8N1, PIN_SENSOR_TX, PIN_SENSOR_RX);   // UART for monitoring the radar
  delay(1000);
#ifdef DEBUG_IN_SERIAL                                                // условная компиляция при выводе отладки в порт       
  ...
  if(radar_sensor.begin(Serial2)) { Serial.println(F(" OK")); }       // рапортуем в порт об успешности/не успешности инициализации сенсора
    else { Serial.println(F(" Sensor not connected")); }                  
#else
